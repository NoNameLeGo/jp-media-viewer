...
## Commit Convention
Commit messages use the same conventional commit format (`type(scope): 主题`), with subject and body written in **Chinese**. Footer unchanged: `由AI生成` then `Generated by AI`.

Example:
```
fix(scanner): 修复扫描取消被误计为失败目录
