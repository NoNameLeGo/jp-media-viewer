...
## Commit Convention
Commit messages are written in **Chinese** (subject + body), following conventional commit format (`type(scope): 主题`). The footer includes `由AI生成` followed by `Generated by AI` on the next line.

Example:
```
fix(scanner): 处理扫描取消时的计数错误

协程取消不再被误统计为失败目录，仅记录真实的文件访问失败。
